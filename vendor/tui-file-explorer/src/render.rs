...
fn render_header(explorer: &FileExplorer, frame: &mut Frame, area: Rect, theme: &Theme) {
    ...
    // Truncate from the left at Unicode character boundaries and according to
    // terminal display width. `str::len()` counts UTF-8 bytes, not columns.
    let inner_width = area.width.saturating_sub(4) as usize;
    let display_path = truncate_path(&path_str, inner_width);
...
fn truncate_path(path: &str, width: usize) -> String {
    if width <= 3 || UnicodeWidthStr::width(path) <= width {
    ...
    for (index, character) in path.char_indices().rev() {
        ...
        if consumed + character_width > suffix_width {
            break;
        }
        consumed += character_width;
        boundary = index;
    }
    format!("\u{2026}{}", &path[boundary..])
}
...
mod tests {
    ...
    #[test]
    fn render_header_with_chinese_path_does_not_panic() {
        let mut explorer = FileExplorer::new(std::path::PathBuf::from("C:/中文目录/更深"), vec![]);
        let mut terminal = Terminal::new(TestBackend::new(9, 12)).unwrap();

        terminal
            .draw(|frame| render(&mut explorer, frame, frame.area()))
            .unwrap();
    }
}
