...
> Built for one workflow — mine. The 4-pane layout below mirrors how I actually code all day: files on the left, agent + shell on the right, git status glancing back at me. A native two-pane file manager and a `gix`-powered Git snapshot handle files/git in-process; the rest is PTY sub-panes for agents, shells, and `bottom`.
...
- **Top-left** — native dual-pane file manager (based on `tui-file-explorer`), with a modal Alt+V viewer for the file under the cursor (markdown / code / image).
- **Bottom-left** — native Git snapshot (via `gix` / `gix-diff` + tree-sitter highlighting) that refreshes when the files column changes directory.
...
- **Bundled essentials** — `bottom` ships in every release archive and self-installs into `~/.rimeterm/` on first launch. Files/git are native, no external binaries required.
