...
/// [`crate::markdown::WrappedTextLayout`] so the draw loop can
...
pub fn update_text_layouts(
    blocks: &[DocBlock],
    text_layouts: &mut std::collections::HashMap<TextBlockId, crate::markdown::WrappedTextLayout>,
    content_width: u16,
) -> bool {
    ...
    for block in blocks {
        ...
        } = block
        {
            ...
            text_layouts.insert(
                *id,
                crate::markdown::WrappedTextLayout {
                    wrapped,
                    physical_to_logical,
                },
            );
...
fn physical_row_source(
    t: &TableBlock,
    local: usize,
    layout: Option<&crate::markdown::TableLayout>,
) -> u32 {
...
pub fn source_line_at(
    blocks: &[DocBlock],
    visual_row: u32,
    text_layouts: &std::collections::HashMap<TextBlockId, crate::markdown::WrappedTextLayout>,
    table_layouts: &std::collections::HashMap<TableBlockId, crate::markdown::TableLayout>,
) -> u32 {
    let mut offset = 0u32;
    for block in blocks {
        let h = block.height();
...
///
/// ```
/// # use rimeterm_markdown::markdown::{DocBlock, TextBlockId, logical_line_at_source};
...
///
/// ```
/// # use rimeterm_markdown::markdown::heading_to_anchor;
...
// ---------------------------------------------------------------------------
// Wrap / table layout caches
//
// Upstream markdown-reader defined these under `crate::ui::markdown_view::state`
// (the widget layer). The widget layer is NOT part of this extraction, but the
// data types themselves are pure and are referenced from the render helpers
// above (`update_text_layouts`, `source_line_at`, `logical_line_at_source`,
// `cell_to_string`) — so they live here instead. Consumers own the caches.
// ---------------------------------------------------------------------------

/// Cached rendered layout for a `DocBlock::Table` at the current viewport
/// width.
///
/// `physical_to_source[i]` is the 0-indexed source line for physical row
/// `i` of the rendered table (counting from the top border). Its length
/// always equals `text.lines.len()`.
#[derive(Debug)]
...
/// Cached wrap output for a single `DocBlock::Text` at the current layout
/// width.
///
/// `wrapped[i]` is the `i`-th physical output row after greedy word-wrap.
/// `physical_to_logical[i]` is the 0-based index into
/// `DocBlock::Text::text.lines` (the *logical* lines) that physical row
/// `i` comes from.
///
/// Invariants:
/// - `wrapped.len() == physical_to_logical.len()`.
/// - `physical_to_logical` is non-decreasing.
#[derive(Debug)]
pub struct WrappedTextLayout {
    ...
    /// `physical_to_logical[i]` — the logical-line index within
    /// `DocBlock::Text::text.lines` that physical row `i` comes from.
