...
impl App {
    pub fn new(workspace_root: PathBuf, config: Config) -> Result<Self> {
        ...
        let git_members = vec![git_pane_id];
    ...
    fn poll_pane_background(&mut self) -> bool {
        ...
        self.sync_file_manager_cwd_to_git();
        dirty
    }

    /// Forward the file manager's active directory to the Git pane so
    /// its snapshot tracks navigation.
    fn sync_file_manager_cwd_to_git(&mut self) {
        ...
        let current = fm.active_dir().to_path_buf();
        if self.last_file_manager_cwd.as_deref() == Some(current.as_path()) {
            return;
        }
        self.last_file_manager_cwd = Some(current.clone());
        if let Some(pane) = self.panes.get_mut(git_id) {
            if let Some(git) = pane
                .as_any_mut()
                .and_then(|any| any.downcast_mut::<crate::git_pane::GitPane>())
            {
                git.refresh_for(&current);
            }
        }
    }
...
mod tests {
    ...
    fn tools_install_of_essential_returns_already_bundled() {
        ...
        let out =
            run_tool_action(ToolAction::Install, bottom_spec).expect("essentials never fail");
