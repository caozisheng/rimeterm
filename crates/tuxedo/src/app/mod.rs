...
pub struct App {
    ...
    /// Theme used by renderers; embedding swaps this value for one draw.
    render_theme: Theme,
...
impl App {
    ...
    fn from_store(store: Store, file_path: PathBuf, cfg: Config) -> Self {
        ...
        let saved_filters = cfg
            ...
            .collect();
        let prefs = Prefs::from_config(cfg);
        let render_theme = *prefs.theme();
        let mut app = Self {
            store,
            view: View::List,
            mode: Mode::Normal,
    ...
    /// Replace the theme used by renderers, returning the previous value.
    pub(crate) fn replace_render_theme(&mut self, theme: Theme) -> Theme {
        std::mem::replace(&mut self.render_theme, theme)
    }
    ...
    pub fn poll_external_changes(&mut self) -> bool {
        ...
        let mut changed = !matches!(reconcile, Reconcile::Unchanged);
        if matches!(reconcile, Reconcile::Reloaded) {
            self.on_reload();
        } else if matches!(reconcile, Reconcile::ReadError) {
            self.flash("read failed");
        }
        let report = self.store.drain_inbox();
        changed |= report.merged > 0 || report.skipped > 0 || report.error.is_some();
