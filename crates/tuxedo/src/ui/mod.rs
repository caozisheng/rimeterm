...
/// Render the standalone full-screen UI using the app-selected theme.
pub fn draw(frame: &mut Frame, app: &mut App) {
    let theme = *app.prefs.theme();
    draw_in(frame, frame.area(), app, &theme);
}

/// Render the UI inside `area` using a caller-supplied theme.
///
/// Cells outside `area` are never touched. This is the embedding boundary;
/// every layout and overlay calculation is relative to the caller rectangle.
pub fn draw_in(frame: &mut Frame, area: Rect, app: &mut App, theme: &crate::theme::Theme) {
    let previous_theme = app.replace_render_theme(*theme);
    ...
    // Preserve at least MIN_BODY_W for tasks. Side panes appear only when the
    // caller rectangle has enough surplus width; embedded rimeterm panes are
    // commonly narrower than a standalone terminal.
    let mut surplus = area.width.saturating_sub(MIN_BODY_W);
    let show_left = app.prefs.layout.left && surplus >= LEFT_PANE_W;
    if show_left {
        surplus -= LEFT_PANE_W;
    }
    let show_right = app.prefs.layout.right && surplus >= RIGHT_PANE_W;
    ...
    app.replace_render_theme(previous_theme);
}
