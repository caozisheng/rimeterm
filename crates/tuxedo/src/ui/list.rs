...
pub fn render(frame: &mut Frame, area: Rect, app: &mut App) {
    ...
    if app.tasks().is_empty() {
        ...
        app.set_text_layout(body_area, Vec::new(), None);
        return;
    }
    ...
    if visible.is_empty() {
        ...
        for (i, (&abs, gk)) in visible.iter().zip(groups.iter()).enumerate() {
            ...
            let opts = task_row::RowOpts {
                ...
                match_term: (!app.filter.search.is_empty()).then_some(app.filter.search.as_str()),
                today: app.today(),
...
fn group_count_key(gk: &GroupKey) -> String {
    match gk {
        ...
        GroupKey::ListContext(Some(name)) => format!("ctx:{name}"),
        GroupKey::ListContext(None) => "ctx:_".to_string(),
...
fn group_header<'a>(theme: &Theme, gk: &GroupKey, count: usize) -> Line<'a> {
    let (label, color) = match gk {
        ...
        GroupKey::ListContext(Some(name)) => (format!("@{name}"), theme.context),
        GroupKey::ListContext(None) => ("NO CONTEXT".to_string(), theme.dim),
