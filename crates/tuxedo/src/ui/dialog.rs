...
mod tests {
    ...
    fn autocomplete_popup_hidden_when_no_token() {
        ...
        let mut app = build_insert_app("(A) 2026-05-01 a +uniqueprojname\n", "plain text");
        let backend = TestBackend::new(80, 30);
        let mut terminal = Terminal::new(backend).unwrap();
        terminal.draw(|f| crate::ui::draw(f, &mut app)).unwrap();
        let popup = popup_region_text(terminal.backend().buffer());
    ...
    fn autocomplete_popup_filters_by_context_kind() {
        let mut app =
            build_insert_app("(A) 2026-05-01 a +uniqueprojname @uniquecontext\n", "Foo @");
        let backend = TestBackend::new(80, 30);
        let mut terminal = Terminal::new(backend).unwrap();
        terminal.draw(|f| crate::ui::draw(f, &mut app)).unwrap();
        let popup = popup_region_text(terminal.backend().buffer());
