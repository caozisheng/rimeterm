...
fn snapshot_app(name: &str, app: &mut App) {
    let buf = render(app);
...
#[test]
fn list_default() {
    snapshot_app("list_default", &mut make_app());
}
...
fn list_with_search() {
    ...
    snapshot_app("list_with_search", &mut app);
}
...
fn list_with_project_filter() {
    ...
    snapshot_app("list_with_project_filter", &mut app);
}

#[test]
fn list_grouped_by_due() {
    let mut app = make_app();
    // Exercise the Due grouping path without writing user configuration.
    let cfg = Config {
        sort: Some(tuxedo::app::Sort::Due),
        density: Some(Density::Compact),
        ..Config::default()
    };
    app.reload_config(cfg);
    app.flash("sort: due");
    snapshot_app("list_grouped_by_due", &mut app);
}
...
fn list_no_sidebars() {
    ...
    snapshot_app("list_no_sidebars", &mut app);
}
...
fn list_sidebar_empty_hints() {
    ...
    app.config_path = Some(PathBuf::from(FIXTURE_CONFIG_PATH));
    app.prefs.density = Density::Compact;
    snapshot_app("list_sidebar_empty_hints", &mut app);
}
...
fn archive_view() {
    ...
    snapshot_app("archive_view", &mut app);
}
...
fn help_overlay() {
    let mut app = make_app();
    app.mode = Mode::Help;
    snapshot_app("help_overlay", &mut app);
}
...
fn settings_overlay() {
    ...
    snapshot_app("settings_overlay", &mut app);
}
...
fn command_palette_unfiltered() {
    ...
    snapshot_app("command_palette_unfiltered", &mut app);
}
...
fn command_palette_filtered() {
    ...
    snapshot_app("command_palette_filtered", &mut app);
}
...
fn command_palette_preserves_visual_selection() {
    ...
    app.mode = Mode::CommandPalette;
    snapshot_app("command_palette_preserves_visual_selection", &mut app);
}
...
fn insert_dialog() {
    ...
    snapshot_app("insert_dialog", &mut app);
}
...
fn insert_dialog_after_nl_parse() {
    ...
    snapshot_app("insert_dialog_after_nl_parse", &mut app);
...
fn insert_slash_menu() {
    ...
    snapshot_app("insert_slash_menu", &mut app);
}
...
fn insert_calendar_for_due() {
    ...
    snapshot_app("insert_calendar_for_due", &mut app);
}
...
fn insert_recurrence_builder() {
    ...
    snapshot_app("insert_recurrence_builder", &mut app);
}
...
fn insert_priority_chooser() {
    ...
    snapshot_app("insert_priority_chooser", &mut app);
}
...
fn empty_state() {
    ...
    app.prefs.layout.left = false;
    app.prefs.layout.right = false;
    snapshot_app("empty_state", &mut app);
}
...
fn welcome_overlay() {
    ...
    snapshot_app("welcome_overlay", &mut app);
}
...
fn render_text(app: &mut App, cols: u16, rows: u16) -> String {
    let backend = TestBackend::new(cols, rows);
    let mut terminal = Terminal::new(backend).expect("terminal init");
    terminal.draw(|f| ui::draw(f, app)).expect("draw frame");
...
fn list_scrolls_to_keep_cursor_visible_when_below_fold() {
    ...
    let text = render_text(&mut app, 80, 12);
    assert!(
        text.contains(&label),
