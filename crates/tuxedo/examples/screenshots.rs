...
fn main() -> std::io::Result<()> {
    ...
    save(&mut app, &out.join("command-palette.svg"))?;
    ...
    save(&mut app, &out.join("empty.svg"))?;
    ...
    for (i, t) in theme::BUILT_IN.iter().enumerate() {
        ...
        save(&mut app, &out.join(format!("theme-{slug}.svg")))?;
    }
...
fn save(app: &mut App, path: &Path) -> std::io::Result<()> {
