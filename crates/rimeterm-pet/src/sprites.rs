use crate::agent_link::AgentPhase;
...
pub fn sprite_to_braille(sprite: &Sprite) -> String {
    ...
    for cell_row in 0..4 {
        for cell_col in 0..8 {
            ...
            let px = cell_col * 2;
            let py = cell_row * 4;
            if pixel(sprite, px, py) {
                code |= 0x01;
            }
            if pixel(sprite, px, py + 1) {
                code |= 0x02;
            }
            if pixel(sprite, px, py + 2) {
                code |= 0x04;
            }
            if pixel(sprite, px + 1, py) {
                code |= 0x08;
            }
            if pixel(sprite, px + 1, py + 1) {
                code |= 0x10;
            }
            if pixel(sprite, px + 1, py + 2) {
                code |= 0x20;
            }
            if pixel(sprite, px, py + 3) {
                code |= 0x40;
            }
            if pixel(sprite, px + 1, py + 3) {
                code |= 0x80;
            }
...
/// Render an agent-specific pose while preserving pet lifecycle priority.
pub fn agent_sprite(state: &PetState, phase: AgentPhase, frame: u8) -> String {
    let base = select_sprite(state);
    if !state.is_alive || state.stage == LifeStage::Egg || state.is_sleeping || state.is_sick {
        return sprite_to_braille(base);
    }
    let sprite = match phase {
        AgentPhase::Busy => {
            if frame % 2 == 0 {
                &WORKING_A
            } else {
        ...
        AgentPhase::Active => &WORKING_B,
        AgentPhase::Completed => &CELEBRATE,
        ...
        | AgentPhase::Exited
        | AgentPhase::Stale
        | AgentPhase::MonitorStale => base,
    };
    sprite_to_braille(sprite)
}

#[rustfmt::skip]
const WORKING_A: Sprite = [0,0b0000000110000000,0b0000011111100000,0b0000111111110000,0b0001111111111000,0b0001101111011000,0b0001111111111000,0b0001111001111000,0b0001111111111000,0b0000111111110000,0b0000011111100000,0b0000001111000000,0b0000110001100000,0b0001100000110000,0,0];
#[rustfmt::skip]
const WORKING_B: Sprite = [0,0b0000000110000000,0b0000011111100000,0b0000111111110000,0b0001111111111000,0b0001101111011000,0b0001111111111000,0b0001111001111000,0b0001111111111000,0b0000111111110000,0b0000011111100000,0b0000001111000000,0b0000011001100000,0b0000011001100000,0,0];
const SPAWNING_A: Sprite = WORKING_A;
const SPAWNING_B: Sprite = WORKING_B;
#[rustfmt::skip]
const CELEBRATE: Sprite = [0,0b0001100110011000,0b0000011111100000,0b0000111111110000,0b0001111111111000,0b0001101111011000,0b0001111111111000,0b0001110000111000,0b0001111111111000,0b0000111111110000,0b0000011111100000,0b0000001111000000,0b0000110001100000,0b0001100000110000,0,0];
#[rustfmt::skip]
const WAITING: Sprite = [0,0b0000000110000000,0b0000011111100000,0b0000111111110000,0b0001111111111000,0b0001101111011000,0b0001111111111000,0b0001111001111000,0b0001111111111000,0b0000111111110000,0b0000011111100000,0b0000001111000000,0b0000010000100000,0b0000010000100000,0,0];
...
mod tests {
    ...
    #[test]
    fn busy_and_idle_use_different_agent_sprites() {
        let mut state = PetState::new_egg(Utc::now());
        state.stage = LifeStage::Child;
        state.character = Character::Marutchi;
        assert_ne!(
            agent_sprite(&state, AgentPhase::Busy, 0),
            agent_sprite(&state, AgentPhase::Idle, 0)
        );
    }

    #[test]
    fn busy_agent_sprite_animates_between_frames() {
        let mut state = PetState::new_egg(Utc::now());
        state.stage = LifeStage::Child;
        state.character = Character::Marutchi;
        assert_ne!(
            agent_sprite(&state, AgentPhase::Busy, 0),
            agent_sprite(&state, AgentPhase::Busy, 1)
        );
    }
