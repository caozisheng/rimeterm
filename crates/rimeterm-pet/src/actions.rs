...
impl std::fmt::Display for ActionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            Self::PetIsDead => "pet is dead",
            Self::PetIsSleeping => "pet is sleeping",
            Self::PetIsSick => "pet is sick",
            Self::PetIsNotSick => "pet is not sick",
            Self::NoDisciplineCallPending => "no discipline call pending",
            Self::NoPoop => "no poop to clean",
        })
    }
}
