...
impl ZoneHandle {
    ...
    /// seconds-east-of-UTC value so `UTC+0` and `GMT` collapse.
    ...
    /// DST-aware for named zones at `now_utc`.
...
/// Format a UTC offset as `UTC+N` or `UTC+N:MM`.
...
mod tests {
    ...
    #[test]
    fn utc_and_gmt_map_to_zero_offset() {
        assert_eq!(
            parse_zone("UTC").unwrap(),
            ZoneHandle::Fixed(FixedOffset::east_opt(0).unwrap())
        );
        assert_eq!(
            parse_zone("gmt").unwrap(),
            ZoneHandle::Fixed(FixedOffset::east_opt(0).unwrap())
        );
    }

    #[test]
    fn explicit_offset_parses_hours_and_minutes() {
        let handle = parse_zone("UTC+5:30").unwrap();
        assert_eq!(
            handle,
            ZoneHandle::Fixed(FixedOffset::east_opt(5 * 3600 + 30 * 60).unwrap())
        );
    }

    #[test]
    fn named_zone_round_trips_through_chrono_tz() {
        let handle = parse_zone("Asia/Shanghai").unwrap();
        assert!(matches!(
            handle,
            ZoneHandle::Named(chrono_tz::Asia::Shanghai)
        ));
    }

    #[test]
    fn identity_keys_dedup_utc_and_gmt() {
        let utc = parse_zone("UTC").unwrap();
        let gmt = parse_zone("GMT").unwrap();
        assert_eq!(utc.identity_key(), gmt.identity_key());
    }

    #[test]
    fn unknown_zone_reports_the_input() {
        let err = parse_zone("Mars/Olympus_Mons").unwrap_err();
        assert!(matches!(&err, TimezoneError::Unknown(s) if s == "Mars/Olympus_Mons"));
    }

    #[test]
    fn offset_format_short_omits_minutes_when_zero() {
        assert_eq!(format_utc_offset_short(0), "UTC+0");
        assert_eq!(format_utc_offset_short(-4 * 3600), "UTC-4");
        assert_eq!(format_utc_offset_short(5 * 3600 + 30 * 60), "UTC+5:30");
    }
}
