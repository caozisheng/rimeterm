...
use crate::coords::ZONE_COORDS;
use crate::handle::ZoneHandle;
use crate::projection::wrap_lon;
...
mod tests {
    ...
    #[test]
    fn shanghai_resolves_to_the_expected_point() {
        let loc = locate(&parse_zone("Asia/Shanghai").unwrap(), now());
        assert_eq!(loc.placement, Placement::Geographic);
        assert!((loc.lat - 31.23).abs() < 0.1);
        assert!((loc.lon - 121.47).abs() < 0.1);
    }
